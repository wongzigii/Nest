...
///
/// (20, ##__VA_ARGS__) is tricky here: 20 is metamacros's largest argument
/// number, ##__VA__ARGS__ eliminates itself when __VA__ARGS__ has nothing.
...
#define ObjCDynamicPropertyGetter(RETURN_TYPE, ...) \
    _KEYWORD_FILE_SCOPE \
        ...
        _MODULE_CONSTRUCTOR_HIGH_PRIORITY \
...
///
/// A simple simulation for the following decision tree:
///
/// ```
///     NOTHING
///   /         \
...
NS_SWIFT_UNAVAILABLE("_ObjCDynamicPropertySynthesizerAddGetter is unavailable in Swift, use ObjCDynamicPropertySynthesizerAddSetter instead.");
